...
void Sphere_intersect_with_ray(Shape *SphereSelf, Ray *r)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        r->xs->intersects[*count - 1] =
            mark_intersect((i == 0) ? t1 : t2, (Shape *)self);
    }
}
void Sphere_set_transform(Shape *selfPtr, Matrix_4x4 *transformMatPtr)
{
    Sphere *self = (Sphere *)selfPtr;
    *self->shape.transform = simd_mul(*transformMatPtr, *self->shape.transform);
}
Vector Sphere_surface_normal_at(const Shape *selfPtr, Point *worldPoint)
{
    if (worldPoint->w != 1)
    {
    ...
    Vector worldNormal =
        Shape_local_to_world_normal(&self->shape, &localNormal);
    return worldNormal;
}
